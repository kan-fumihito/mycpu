...
class Core extends Module {
  ...
  val wb_data = MuxCase(alu_out, Seq((inst === LW) -> io.dmem.rdata))
  when(
    inst === LW || inst === ADD || inst === ADDI || inst === SUB || inst === AND || inst === OR || inst === XOR || inst === ANDI || inst === ORI || inst === XORI
  ) {
  ...
  printf(p"rs1_addr : 0x$rs1_addr\n")
  printf(p"rs2_addr : 0x$rs2_addr\n")
  printf(p"wb_addr  : 0x$wb_addr\n")
